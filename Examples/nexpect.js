    ...
    run: function (callback) {
      ...
      function validateFnType(currentFn) {
        if (typeof currentFn !== "function") {
          ...
          //
          onError(
            new Error("Cannot process non-function on nexpect stack."),
            true
          );
          return false;
        } else if (
          ["_expect", "_sendline", "_wait", "_sendEof"].indexOf(
            currentFn.name
          ) === -1
        ) {
          //
          ...
          //
          onError(
            new Error("Unexpected context function name: " + currentFn.name),
            true
          );
          return false;
        }
      ...
      function evalContext(data, name) {
        ...
        if (
          !currentFn ||
          (name === "_expect" && currentFn.name === "_expect")
        ) {
          //
        ...
        if (currentFn.name === "_expect") {
          ...
          return currentFn(data) === true
            ...
            : onError(
                createExpectationError(currentFn.expectation, data),
                true
              );
        ...
        if (!lastLine) {
          onError(
            createUnexpectedEndError(
              "No data from child with non-empty queue.",
              remainingQueue
            )
          );
          return false;
          ...
          onError(
            new Error("Cannot call sendline after the process has exited")
          );
          return false;
      ...
      if (!/^win/.test(process.platform)) {
        ...
        context.process = spawn(
          "cmd",
          ["/c", `${context.command}`].concat(context.params),
          options
        );
      }
...
function createUnexpectedEndError(message, remainingQueue) {
  let desc = remainingQueue.map(function (it) {
